class RakeMKV::Parser
  ...
  def sinfo
    ...
    guide('SINFO') do |line|
      ...
      @sinfo[title][specific] = Hash.new unless @sinfo[title][specific].is_a? Hash
  ...
  def messages
    @messages = cleaned.collect do |line|
      split = line[0].split(':')
  ...
  def drives
    @drives = cleaned.collect do |line|
      if line[0].split(':').first == 'DRV'
        { accessible: accessible(line), drive_name: line[4], disc_name: line[5] }
      end
    end
    @drives.compact!
    @drives
